$(document).ready(() => {
  ...
  /* Filter Place */
  $('button').click(() => {
    const amenR = { amenities: [] };
    $('article').remove();
    for (const key of Object.keys(amenCheckList)) {
      amenR.amenities.push(key);
    }
    $.ajax({
      ...
      data: JSON.stringify(amenR),
      success: data => articles(data)
    });
  });
  ...
  const articles = data => {
    $('.places h1').html('Total places: ' + data.length);
    data.map(place => {
      $('.places h1').after(`
      <article>
        <div class="title_box">
